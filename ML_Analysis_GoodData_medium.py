...
import json
from ML_Analysis_mediumdata import LoadMediumData


class TakeLogFromCM(LoadMediumData):
    ...
    def mlanalysis(self, x_data, y_data):
        ...
        for _, (train, test) in enumerate(kf.split(X,Y)):
            ...
            for i in tqdm(range(len(test))):
                ...
                this_hit = 1 if self.take_hit_bool(this_index) else 0
                self.log_dict[f"true_{this_gt}"][f"pred_{this_predict}"].append([this_hit, this_meta])
...
if __name__=='__main__':
    ...
    x = mydata.take_x()
    y = mydata.take_y()
    # print(x.columns.tolist().index('gaze_hit'))
    mydata.mlanalysis(x,y)
    with open("ml-results/medium/RF_task2_CM_meta.json", "w") as f:
